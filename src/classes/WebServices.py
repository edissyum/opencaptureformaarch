...
class WebServices:
    ...
    def check_document(self, chrono):
        ...
        try:
            ...
            res = requests.post(self.baseUrl + 'res/list', auth=self.auth, 
                data=args,
    ...
    def reattach_to_document(self, res_id_origin, res_id_signed, typist, config):
        ...
        try:
            ...
            if res.status_code != 204:
                self.Log.error('(' + str(res.status_code) + ') Reattach to document : ' + str(res.text))
                return False
            else:
                return True
        except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e:
    ...
    def change_status(self, res_id, config):
        ...
        try:
            ...
            res = requests.put(
                self.baseUrl + 'res/resource/status',
                auth=self.auth, data=args,
                headers={'Connection': 'close', 'Content-Type': 'application/json'
            })

            if res.status_code != 200:
                self.Log.error('(' + str(res.status_code) + ') change status of the document : ' + str(res.text))
                return False
            else:
                return True
        except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e:
